...
import type { CatalogModelStatus, CollectorAuthKind, Criticality } from "@/types";
      ...
      <header className="onb-top">
        <div className="onb-top-left">
          ...
          <div>
            ...
            <div className="onb-top-sub">
              Register infrastructure &amp; application targets · auto-discovery never auto-commits
            </div>
          </div>
        </div>
        <div className="onb-modes">
          {(
            ...
            <button
              type="button"
              key={m}
              className={`onb-mode${mode === m ? " active" : ""}`}
...
function ManualWizard({ onDone }: { onDone: () => void }) {
  ...
  const stepValid = (s: number): boolean => {
    switch (s) {
      case 0:
        return Boolean(d.tenant && d.site && d.environment);
      case 1:
        return Boolean(d.category && d.vendor && d.model && d.firmware);
      case 2:
        return Boolean((d.host || d.ip) && d.service && d.ownerTeam && d.criticality);
      case 3: {
        const p = profile;
        if (!p) return false;
    ...
    return (
      <div className="onb-done">
        ...
        <p>
          ...
          <b>{d.environment}</b> collector. A validation job will confirm reachability before the
          asset transitions <span className="mono">pending_review → active</span>.
        </p>
        <div className="onb-audit">
          ...
          <span className="mono">{TENANTS.find((t) => t.id === d.tenant)?.name}</span> · secret
          stored as Vault reference only
        </div>
    ...
    <div className="onb-wizard">
      ...
      <ol className="onb-steps">
        {STEPS.map((s) => (
          <li
            key={s.id}
            className={`onb-step${s.id === step ? " current" : ""}${s.id < step ? " done" : ""}`}
          >
            <button
              type="button"
        ...
        {step === 0 && (
          <div className="onb-grid">
              ...
              <div className="onb-seg">
                {ENVIRONMENTS.map((e) => (
                  <button
                    type="button"
                    key={e}
                    className={`onb-seg-btn${d.environment === e ? " active" : ""}`}
            ...
            <div className="onb-note">
              <i className="fa-solid fa-lock" /> The tenant, site and environment are bound into
              every Prometheus target label and Alertmanager route — they cannot be changed after
              onboarding without a new record.
            </div>
          </div>
        )}
              ...
              <div className="onb-cat-grid">
                {DEVICE_CATEGORIES.map((c) => (
                  <button
                    ...
                    onClick={() =>
                      setD((p) => ({ ...p, category: c.id, vendor: "", model: "", firmware: "" }))
                    }
                  >
            ...
            {model && (
              <div className="onb-model-card span2">
                <div className="onb-model-head">
                  <Pill kind={STATUS_PILL[model.status]}>
                ...
                <div className="onb-caps">
                  {model.capabilities.map((c) => (
                    <span key={c} className="onb-cap mono">
                      {c}
                    </span>
                  ))}
                </div>
        ...
        {step === 2 && (
          <div className="onb-grid">
            <TextField
              ...
              placeholder="e.g. sw-blr-tor-15.finspot.in"
              mono
            />
            <TextField
              label="Management IP"
            ...
            <TextField
              label="Owner team"
              value={d.ownerTeam}
              onChange={(v) => set("ownerTeam", v)}
              placeholder="e.g. Network Eng"
            />
            <div className="onb-field span2">
              ...
              <div className="onb-crit">
                {CRITICALITY_TIERS.map((t) => (
                  <button
                    type="button"
                    key={t.id}
                    className={`onb-crit-btn crit-${t.id}${d.criticality === t.id ? " active" : ""}`}
                    onClick={() => set("criticality", t.id)}
                  >
                    <b>{t.label}</b>
                    <span>{t.sla}</span>
                  </button>
        ...
        {step === 3 && (
          <div className="onb-grid">
              ...
              <div className="onb-field">
                ...
                <div className="onb-caps">
                  {profile.metrics.map((m) => (
                    <span key={m} className="onb-cap mono">
                      {m}
                    </span>
                  ))}
                </div>
                ...
                <div className="onb-sec-grid">
                    ...
                    <div className="onb-seg">
                      {SNMP_LEVELS.map((l) => (
                        <button
                          type="button"
                          key={l}
                          className={`onb-seg-btn${d.snmpLevel === l ? " active" : ""}`}
                          onClick={() => set("snmpLevel", l)}
                        >
                          {l}
                        </button>
                      ))}
                  ...
                  {d.snmpLevel !== "noAuthNoPriv" && (
                    <SelectField
                      label="Auth method"
                      value={d.snmpAuth}
                      onChange={(v) => set("snmpAuth", v as Draft["snmpAuth"])}
                      options={SNMP_AUTH.map((a) => ({ value: a, label: a }))}
                    />
                  )}
                  {d.snmpLevel === "authPriv" && (
                    <SelectField
                      label="Privacy method"
                      value={d.snmpPriv}
                      onChange={(v) => set("snmpPriv", v as Draft["snmpPriv"])}
                      options={SNMP_PRIV.map((a) => ({ value: a, label: a }))}
                    />
                  )}
                </div>
              </div>
            ...
            {profile?.requiresVault ? (
              <TextField
                label="Vault credential reference"
                value={d.vaultRef}
                onChange={(v) => set("vaultRef", v)}
                placeholder="vault:kv/<tenant>/<site>/<asset>"
                mono
                span2
              />
            ) : (
              <div className="onb-note span2">
                <i className="fa-solid fa-circle-info" /> This collector needs no stored secret (
                {profile?.authKind ?? "none"}).
              </div>
            )}
            <div className="onb-note span2 onb-note-warn">
              <i className="fa-solid fa-shield-halved" /> Secrets are never stored in LinkedEye —
              only the Vault path is persisted. SNMP community strings, SNMPv3 passwords and Redfish
              credentials live in Vault exclusively.
            </div>
          </div>
        )}

        {step === 4 && <ReviewPanel d={d} />}
      </div>
      ...
      <div className="onb-foot">
        ...
        <div className="onb-foot-status">
          ...
          {!stepValid(step) && (
            <span className="onb-foot-hint"> · complete required fields to continue</span>
          )}
        </div>
        {step < STEPS.length - 1 ? (
          <button
...
function ReviewPanel({ d }: { d: Draft }) {
  ...
  const rows: [string, string][] = [
    ...
    [
      "Auth",
      profile
        ? isSnmpV3(profile.authKind)
          ? `SNMPv3 ${d.snmpLevel} (${d.snmpAuth}/${d.snmpPriv})`
          : profile.authKind
        : "—",
    ],
...
const BULK_ROWS = [
  ...
  { row: 4, host: "fw-nyc-dmz-03", ip: "10.30.0.9", model: "Palo Alto PA-3260", ok: true, err: "" },
  {
    ...
    ok: false,
    err: "duplicate management IP (already 10.20.5.9 → LB-FRA-PROD-03)",
  },
  {
    row: 6,
    host: "unknown-dev-1",
    ...
    model: "Cisco C9500",
    ok: false,
    err: "invalid IPv4 address 10.10.3.999",
  },
];
  ...
  return (
    <div className="onb-bulk">
      <div className="onb-bulk-drop">
        ...
        <div>
          <b>Drop a CSV or JSON manifest</b>
          <div className="text-mute" style={{ fontSize: 12 }}>
            Columns: tenant, site, environment, host, ip, vendor, model, firmware, service,
            owner_team, criticality, collector_profile, vault_ref
          </div>
        </div>
      ...
      <div className="sn-form-section">
        <div className="sn-section-header">
          ...
          <span className="sn-section-badge">
            {ok} valid · {bad} error{bad === 1 ? "" : "s"}
          </span>
        </div>
        <div className="sn-section-content">
          <p style={{ fontSize: 12, color: "var(--fg-subtle)", marginTop: 0 }}>
            Row-level validation runs before any commit. Invalid rows are surfaced individually —
            the import is never a single pass/fail, and valid rows can be committed while errors are
            fixed.
          </p>
            ...
            <tbody>
              {BULK_ROWS.map((r) => (
                <tr key={r.row}>
                  ...
                  <td>
                    <i
                      className={`fa-solid ${r.ok ? "fa-circle-check" : "fa-circle-exclamation"}`}
                      style={{ color: r.ok ? "#10b981" : "#dc2626" }}
                    />
                  </td>
                  ...
                  <td>
                    {r.ok ? (
                      <Pill kind="success">valid</Pill>
                    ) : (
                      <span style={{ color: "#dc2626", fontSize: 12 }}>{r.err}</span>
                    )}
                  </td>
                </tr>
              ))}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button type="button" className="sn-btn primary" disabled={ok === 0}>
              <i className="fa-solid fa-check" /> Commit {ok} valid row{ok === 1 ? "" : "s"}
            </button>
            <button type="button" className="sn-btn">
      ...
      <div className="onb-disc-list">
        ...
        {DISCOVERY_CANDIDATES.map((c) => (
          <button
            type="button"
            key={c.id}
            className={`onb-disc-item${selected === c.id ? " active" : ""}`}
          ...
          >
            ...
            <div style={{ fontSize: 11, marginTop: 2 }}>
              {c.normalizedVendor} {c.normalizedModel}
            </div>
          </button>
        ))}
      </div>
        ...
        <div className="onb-disc-detail">
          <div className="onb-disc-detail-head">
            <div>
              ...
              <div className="text-mute" style={{ fontSize: 12 }}>
                {cand.ip} · discovered {cand.discoveredAt} via {cand.source}
              </div>
            </div>
            <Pill kind={STATUS_PILL[cand.status]}>
          ...
          <div className="onb-disc-fp">
            ...
            <FpRow
              ...
              v={
                cand.matchedModel
                  ? `${catalogModelById(cand.matchedModel)?.vendor} ${catalogModelById(cand.matchedModel)?.model}`
                  : "no match — generic profile"
              }
            />
          ...
          {!cand.matchedModel && (
            <div className="onb-note onb-note-warn">
              <i className="fa-solid fa-triangle-exclamation" /> Model Review Required — no catalog
              match. A generic safe profile is assigned and destructive automation is disabled until
              a NOC operator maps or creates a model.
            </div>
          )}

          <div className="onb-disc-actions">
            <button type="button" className="sn-btn primary" disabled={cand.confidence < 0.5}>
              <i className="fa-solid fa-check" /> Approve &amp; onboard
            </button>
            <button type="button" className="sn-btn">
              <i className="fa-solid fa-diagram-project" /> Map to existing model
            </button>
            <button type="button" className="sn-btn">
              <i className="fa-solid fa-plus" /> Create new model
            </button>
            <button type="button" className="sn-btn">
              <i className="fa-solid fa-ban" /> Block device
            </button>
          </div>
          <div className="onb-note">
            <i className="fa-solid fa-circle-info" /> Approving writes an immutable audit event and
            moves the asset to <span className="mono">pending_review</span> — a validation job must
            pass before it becomes <span className="mono">active</span>.
          </div>
        </div>
      ) : (
        <div
          className="onb-disc-detail"
          style={{ display: "grid", placeItems: "center", color: "var(--fg-subtle)" }}
        >
...
function ConfidenceDot({ v }: { v: number }) {
  ...
  return (
    <span className="onb-conf" style={{ color }}>
      {Math.round(v * 100)}%
    </span>
  );
}
