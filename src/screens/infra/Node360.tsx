      ...
      <div className="page page-fade narrow">
        ...
        <div className="card">
          <div className="card-body" style={{ padding: 24 }}>
            No asset with id <code>{id}</code>.{" "}
            <Link to="/infra/assets" className="sn-link">
              Back to inventory
            </Link>
            .
          </div>
    ...
    <div className="page page-fade sn-dash">
      ...
      <div className="sn-form-header">
        ...
        <div className="sn-form-title-meta">
          ...
          <div className="sn-form-sub">
            {device.role} · {site?.name ?? device.site} · <span className="mono">{device.ip}</span>{" "}
            · {device.os}
          </div>
        </div>
        <div className="sn-form-actions">
          <Link to="/infra/topology" className="sn-btn">
            <i className="fa-solid fa-circle-nodes" /> View in topology
          </Link>
          <button type="button" className="sn-btn">
            <i className="fa-solid fa-rotate" /> Poll now
          </button>
          <button type="button" className="sn-btn primary">
            <i className="fa-solid fa-wand-magic-sparkles" /> Suggest remediation
          </button>
        </div>
      </div>
      ...
      <div className="n360-metrics">
        <MetricTile
          ...
          value={`${device.cpu}%`}
          series={cpuSeries}
          tone={device.cpu >= 85 ? "crit" : device.cpu >= 70 ? "warn" : "ok"}
        />
        <MetricTile
          label="Memory"
          value={`${device.mem}%`}
          series={memSeries}
          tone={device.mem >= 85 ? "crit" : device.mem >= 70 ? "warn" : "ok"}
        />
        <MetricTile
          label="Throughput"
          value={device.throughput === "—" ? "n/a" : device.throughput}
          series={netSeries}
          tone="ok"
        />
        <MetricTile
          label="Latency p95"
          value={device.status === "warn" ? "28 ms" : "6 ms"}
          series={latSeries}
          tone={device.status === "warn" ? "warn" : "ok"}
        />
        <MetricTile label="Uptime" value={device.uptime} tone="neutral" />
        <MetricTile
          label="Last seen"
          value={device.lastSeen}
          tone={device.status === "down" ? "crit" : "ok"}
        />
      </div>
      ...
      <div className="sn-form-layout">
        <div className="sn-form-main">
          ...
          <Section icon="fa-bell" title="Active alerts" badge={`${alerts.length}`}>
            {alerts.length === 0 ? (
              <p style={{ fontSize: 12.5, color: "var(--fg-subtle)", margin: 0 }}>
                No firing alerts for this device.
              </p>
            ) : (
          ...
          <Section icon="fa-diagram-project" title="Correlated signals" badge="AI · read-only">
            ...
            <p
              style={{ fontSize: 11.5, color: "var(--fg-subtle)", marginTop: 10, marginBottom: 0 }}
            >
              <i
                className="fa-solid fa-circle-info"
                style={{ marginRight: 6, color: "var(--accent)" }}
              />
              Recommendations are advisory. No remediation runs without explicit approval and a
              rollback plan.
            </p>
          </Section>
        </div>
        ...
        <aside className="sn-form-side">
          <div className="sn-side-card">
            ...
            <div className="sn-side-body">
              ...
              <div className="n360-dep-label" style={{ marginTop: 10 }}>
                Downstream ({deps.downstream.length})
              </div>
              {deps.downstream.length === 0 && <p className="n360-dep-empty">— none</p>}
              {deps.downstream.map((dn) => (
                <DepRow key={dn} id={dn} />
              ))}
            </div>
          </div>
          ...
          <div className="sn-side-card">
            ...
            <p style={{ fontSize: 11, color: "var(--fg-subtle)", margin: "8px 4px 0" }}>
              Destructive actions are disabled for{" "}
              {lc === "pending_review" ? "assets pending review" : "this profile"}.
            </p>
          </div>
        </aside>
      </div>
...
function MetricTile({
  ...
  return (
    <div className={`n360-tile tone-${tone}`}>
      ...
      {series && (
        <div className="n360-tile-spark">
          <Sparkline data={series} color={color} w={110} h={30} />
        </div>
      )}
    </div>
  );
...
function correlations(
  id: string,
  status: DeviceStatus,
): { label: string; tone: "ok" | "warn" | "crit"; corr: string }[] {
  const base = [
    {
      label: "CPU saturation ↔ interface error rate",
      tone: (status === "warn" || status === "critical" ? "warn" : "ok") as "ok" | "warn" | "crit",
      corr: "0.82",
    },
    {
      label: "Memory pressure ↔ p95 latency",
      tone: (status === "warn" ? "warn" : "ok") as "ok" | "warn" | "crit",
      corr: "0.74",
    },
    { label: "Recent config change (change window)", tone: "ok" as const, corr: "0.31" },
    {
      label: "Upstream link utilization",
      tone: (status === "critical" ? "crit" : "ok") as "ok" | "warn" | "crit",
      corr: "0.68",
    },
  ];
  return base;
